...
function mapIntention(sentence){
  if(sentence == "How likely is it that the character intentionally opened the door?"){
    return "intention1"
  } else if(sentence == "How likely is it that the character knew how to open the door?"){
    return "knowledge"
  } else if(sentence == "Was the character aiming to open the door?"){
    return "goal"
  } else if(sentence == "Are the character's movements intentional?"){
    return "intention2"
  }else if(sentence ==  "How likely is it that the ball intentionally opened the door?"){
    return "intention1"
  } else if(sentence == "How likely is it that the ball knew how to open the door?"){
    return "knowledge"
  } else if(sentence == "Was the ball aiming to open the door?"){
    return "goal"
  } else if(sentence == "Are the ball's movements intentional?"){
    return "intention2"
  }
...
slides.intentionH = slide({
  ...
  present : randomSort([
    {sentence: "How likely is it that the character intentionally opened the door?"}, //intention
    {sentence: "How likely is it that the character knew how to open the door?"}, //knowledge
    {sentence: "Was the character aiming to open the door?"}, //goal
    {sentence: "Are the character's movements intentional?"} //intention2
  ]),
...
slides.intentionB = slide({
  ...
  /* trial information for this block
   (the variable 'stim' will change between each of these values,
    and for each of these, present_handle will be run.) */
  present : randomSort([
    {sentence: "How likely is it that the ball intentionally opened the door?"}, //intention
    {sentence: "How likely is it that the ball knew how to open the door?"}, //knowledge
    {sentence: "Was the ball aiming to open the door?"}, //goal
    {sentence: "Are the ball's movements intentional?"} //intention2
  ]),
  ...
  present_handle : function(stim) {
    ...
    $(".prompt").html(stim.sentence);
    //erase current value
    document.getElementById('opt1IB').checked = false;
    document.getElementById('opt2IB').checked = false;
    document.getElementById('opt3IB').checked = false;
    document.getElementById('opt4IB').checked = false;
    document.getElementById('opt5IB').checked = false;
    document.getElementById('opt6IB').checked = false;
    document.getElementById('opt7IB').checked = false;
    document.getElementById('opt8IB').checked = false;
    document.getElementById('opt9IB').checked = false;

  },

  button : function() {

    if($('input[name=assessB]:checked').length == 0) {
      $(".err").show();
    } else {
  ...
  log_responses : function() {
    ...
    exp.data[sentence] = $('input[name="assessB"]:checked').val(),
    exp.data[sentenceRT] = this.RT
    exp.data[sentenceOrder] = this.trial_num
...
slides.attention2B =  slide({ //check this!!!
  name : "attention2B",
...
function sendDataToServer(data) {
	$.ajax({
    url: 'https://script.google.com/macros/s/AKfycbyAW6feJXBP4RYGKqfpehPAIxVlfCzWSY1MUuDasZrJQ8GEVQ6VUNZOVaf1Y-VHixtJ/exec',
		type: 'post',
    data: data,
    complete: function(){
      window.location.replace("https://app.prolific.co/submissions/complete?cc=7F794890") // redirect to prolific
    } 
	});
}
